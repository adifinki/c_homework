...
int main()
{
	...
	int ret_val = getchar(); // the \n of the scan
	if (ret_val != '\n')
		ret_value = Input_Error_Handler(User_Option, ILEGAL_INPUT);
		return ret_value;
...
/*___________________definitions________________________*/
...
int Input_Error_Handler(int Users_Option, int ret_value)
{
	if (ret_value == SUCCESS)
		return SUCCESS;
	if (ret_value == EMPTY || ret_value == ILEGAL_INPUT)
	{
		printf("You have entered an illegal input.\n"); //scanf didn't get the wright input format.
	}
	else {
		switch (Users_Option) {
